...
public class SqlDimensionMemberServiceImpl implements DimensionMemberService {
    ...
    public List<MiniCubeMember> getMembers(Cube cube, Level level, DataSourceInfo dataSourceInfo, Member parentMember,
        ...
        QueryRequest queryRequest = buildQueryRequest(cube, queryLevel, parentMember, dataSourceInfo);
        ...
        try {
            ...
            sqlExpression.setNeedColums(
                    SqlColumnUtils.getFacttableColumns(sqlExpression.getQueryMeta(), 
                            queryRequest.getSelect(), queryRequest.getFrom().getFrom(), false, false));
            ...
            sqlExpression.generateNoJoinSql(sqlExpression.getNeedColums(), null, null, andCondition, false);
            SearchIndexResultSet resultSet = 
                    queryHandler.getJdbcHandler().querySqlList(queryHandler.getSqlExpression().getSqlQuery(),
                            sqlExpression.getNeedColums());

            log.info("queryId:{} cost:{} ms in query request.level:{}",
                    QueryRouterContext.getQueryId(),
                    System.currentTimeMillis() - current, queryRequest);
            ...
            log.error("queryId:{} get members error,queryLevel:" + queryLevel + " parentMember:" + parentMember,
                    QueryRouterContext.getQueryId());
    ...
    private List<MiniCubeMember> buildMembersFromCellSet(SearchIndexResultSet resultSet, MiniCubeLevel queryLevel,
        ...
        try {
            ...
            log.info("queryId:{} cost:{} in build dimension:{} member,size:{}", QueryRouterContext.getQueryId(), 
                    System.currentTimeMillis() - current , queryLevel.getDimension().getName(), members.size());
            return Lists.newArrayList(members.values());
        } catch (Exception e) {
            log.error("queryId{} build members error:" + e.getMessage(), QueryRouterContext.getQueryId());
...
//        }
    ...
    public MiniCubeMember getMemberFromLevelByName(DataSourceInfo dataSourceInfo, 
        ...
        log.info("queryId:{} query members,queryRequest:" + queryRequest, QueryRouterContext.getQueryId());
...
//        }
    ...
    public List<MiniCubeMember> getMemberFromLevelByNames(DataSourceInfo dataSourceInfo,
        ...
        try {
            ...
            log.error("queryId:{} error occur when get name:" + uniqueNameList + " from level:" + level,
                    QueryRouterContext.getQueryId());
            throw new MiniCubeQueryException(e);
        }
        return members;
