...
File.prototype = {
	...
	getFolderJSON : function(path, name) {
		try {
			...
			var url ='file.action?mkdir=yes&path=' + encodeURIComponent(path) + '&name=' + encodeURIComponent(name);
...
//		这样可以减少一次请求，有时间再升级
	...
	renameFile : function(trObj, newName) {
		...
		if (name != newName) {
			try {
				...
				var url = 'filerename.action?path=' + encodeURIComponent(path) + '&name=' + encodeURIComponent(newName);
	...
	copyFile : function(trObj) {
		...
		try {
			...
			name = nameTd.firstChild.innerHTML;
			var path = (UPLOAD.uploadPath + decodeHTML(name));
			var url = 'filecopy.action?path=' + encodeURIComponent(path);
	...
	deleteFile : function(trObj) {
		...
		try {
			...
			var info = type == 'folder' ? '您确定要删除 ' + name + ' 文件夹吗？\r\n其全部子文件夹和文件将一同被删除。': 
							'您确定要删除 ' + name + ' 文件吗？';		
			if(confirm(info)) {
				var url = 'filedelete.action?path=' + encodeURIComponent(path);
