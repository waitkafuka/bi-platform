...
define(['template'], function (template){
    ...
    var groupNameInput = '<div class="form-common-line j-data-sources-info-group-name">'
        ...
        +             '<input type="text" name="groupId" value="{{name}}" group-id="{{id}}" readonly="readonly"/>'
        +         '</div>'
        +     '</div>';
    //------------------------------------------
    // html模版
    ...
    var html = '<!--模板-添加数据源-->'
        ...
        +         '<div class="form-common-text form-common-text-big">'
        +             '<input type="text" class="'+ CLASS.J_DATA_SOURCES_NAME +'" name="name" value="{{if ds}}{{ds.name}}{{/if}}"/>'
        ...
        +             '<option value="MYSQL" {{if ds&&ds.dataSourceType== "MYSQL"}} selected="selected"{{/if}}>MySQL</option>'
        +             '<option value="MYSQL-DBPROXY" {{if ds&&ds.dataSourceType== "MYSQL-DBPROXY"}} selected="selected"{{/if}}>MySQL-DBPROXY</option>'
        +             '<option value="HIVE" {{if ds&&ds.dataSourceType== "HIVE"}} selected="selected"{{/if}}>HIVE</option>'
        +             '<option value="PALO" {{if ds&&ds.dataSourceType== "PALO"}} selected="selected"{{/if}}>PALO</option>'
        +         '<select>'
        +     '</div>'
        +     '<div class="form-common-line">'
        +         '<span class="form-common-label">数据库地址：</span>'
        +         '<div class="form-common-text form-common-text-big">'
        +             '<input type="text" class="' + CLASS.J_DATA_SOURCES_ADDRESS + '"name="hostAndPort" value="{{if ds}}{{ds.hostAndPort}}{{/if}}" placeholder="' + STRING.ADDRESS_PLACEHOLDER + '"/>'
        ...
        +         '<div class="form-common-text form-common-text-big">'
        +             '<input type="text" class="' + CLASS.J_INPUT_DATASOURCE_DATABASE + '"name="dbInstance" value="{{if ds}}{{ds.dbInstance}}{{/if}}" placeholder="请输入数据库名"/>'
        ...
        +         '<div class="form-common-text form-common-text-big">'
        +             '<input type="text" class="' + CLASS.J_INPUT_DATASOURCE_USERNAME + '" name="dbUser" value="{{if ds}}{{ds.dbUser}}{{/if}}" placeholder="请输入正确格式的用户名"/>'
        ...
        +         '<div class="form-common-text form-common-text-big">'
        +             '<input type="password" class="' + CLASS.J_INPUT_PASSWORD + '" name="dbPwd" value="{{if ds}}{{ds.dbPwd}}{{/if}}" placeholder="请输入正确格式的密码"/>'
        ...
        +             '{{if ds&&ds.advancedProperties}}{{each ds.advancedProperties as $item}}<div class="j-item"><label>名称：</label><input class="j-item-key" type="text" value={{$index}}><label>属性：</label><input class="j-item-value" type="text" value={{$item}}></div>{{/each}}'
        +             '{{else}}<div class="j-item"><label>名称：</label><input class="j-item-key" type="text" value=""><label>属性：</label><input class="j-item-value" type="text" value=""></div>'
        +             '<div class="j-item"><label>名称：</label><input class="j-item-key" type="text" value=""><label>属性：</label><input class="j-item-value" type="text" value=""></div>'
        +             '<div class="j-item"><label>名称：</label><input class="j-item-key" type="text" value=""><label>属性：</label><input class="j-item-value" type="text" value=""></div>'
        +             '{{/if}}'
        +         '</div>'
        +     '</div>'
