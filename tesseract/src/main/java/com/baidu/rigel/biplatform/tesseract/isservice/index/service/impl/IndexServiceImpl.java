...
public class IndexServiceImpl implements IndexService {
     ...
     *             可能抛出异常
     */
    private void publishIndexUpdateEvent(List<IndexMeta> metaList)
            throws Exception {
        if (metaList != null && metaList.size () > 0) {
            ...
            for (IndexMeta meta : metaList) {
                for (IndexShard idxShard : meta.getIdxShardList ()) {
                    /**
                     * TODO 发出的更新事件中，索引的路径不该有机器节点的indexbase信息，应该是由各机器收到消息后，
                     * 在自己的索引数据目录内进行匹配
                     */
    ...
    private void publistIndexMetaWriteEvent(IndexMeta meta) throws Exception {
        if (meta != null) {
            ...
            LOGGER.info (String.format (
                    ...
                    "post IndexMetaWriteImageEvent with IndexMetaId:"
                            + meta.getIndexMetaId ()));
    ...
    private void doIndexByIndexAction(IndexMeta indexMeta,
        ...
        for (String tableName : sqlQueryMap.keySet ()) {
            ...
            for (int i = 0; i * pcount < total; i++) {
                ...
                TesseractResultSet currResult = this.dataQueryService
                        .queryForDocListWithSQLQuery (sqlQuery,
                                dataSourceWrape, limitStart, limitEnd);
                ...
                while (currResult.size () != 0) {
                    ...
                    // 处理
                    Map<String, Object> result = writeIndex (currResult,
                            idxAction, currIdxShard, isLastPiece,
                            sqlQuery.getIdName ());
                    
                    currResult = (TesseractResultSet) result
                            .get (RESULT_KEY_DATA);
                    currMaxId = (BigDecimal) result.get (RESULT_KEY_MAXID);
                    currIdxShard = (IndexShard) result
                            .get (RESULT_KEY_INDEXSHARD);
                    if (currIdxShard.isFull () || isLastPiece) {
                        ...
                        if (idxAction.equals (IndexAction.INDEX_MOD)
                            ...
                            currIdxShardIdx++;
                            if (currIdxShardIdx >= idxMeta.getIdxShardList ()
                                    .size ()) {
        ...
        if (idxMeta.getIdxState ()
                .equals (IndexState.INDEX_AVAILABLE_NEEDMERGE)) {
    ...
    public Map<String, Object> writeIndex(TesseractResultSet data,
        ...
        LOGGER.info (String.format (
                LogInfoConstants.INFO_PATTERN_FUNCTION_BEGIN, "writeIndex",
                "[data:" + data + "][idxAction:" + idxAction + "][idxShard:"
                        + idxShard + "][lastPiece:" + lastPiece + "][idName:"
                        + idName + "]"));
        
        Node node = this.isNodeService.getNodeByNodeKey (
                idxShard.getClusterName (), idxShard.getNodeKey (),
                Boolean.TRUE);
        
        IndexMessage message = null;
        message = isClient.index (data, idxAction, idxShard, node, idName,
                lastPiece);
        ...
        if (idxShard.isFull () || lastPiece) {
            ...
            for (Node currNode : assignedNodeMap.values ()) {
                int retryTimes = 0;
                // 拷贝到指定的目录
                String targetFilePath = idxShard.getAbsoluteFilePath (currNode
                        .getIndexBaseDir ());
                ServerFeedbackMessage backMessage = null;
                while (retryTimes < TesseractConstant.RETRY_TIMES) {
                    backMessage = isClient
                            .copyIndexDataToRemoteNode (absoluteIdxFilePath,
                                    targetFilePath, true, currNode);
