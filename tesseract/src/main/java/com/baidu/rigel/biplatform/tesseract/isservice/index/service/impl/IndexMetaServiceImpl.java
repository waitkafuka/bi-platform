...
public class IndexMetaServiceImpl extends AbstractMetaService implements
		IndexMetaService {
	/**
	 * LOGGER
	 */
	private static final Logger LOGGER = LoggerFactory
			.getLogger(IndexMetaServiceImpl.class);
	/**
	 * storeManager
	 */
	@Resource
	 ...
	 */
	@Override
	public List<IndexMeta> initMiniCubeIndexMeta(List<Cube> cubeList,
			DataSourceInfo dataSourceInfo) {
		/**
		 * 初始化索引元数据，设置cube的维度、指标、事实表、数据源信息，设置索引元数据状态为UN_INIT;不进行索引分片的分配
		 */
		List<IndexMeta> result = new ArrayList<IndexMeta>();
		if (CollectionUtils.isEmpty(cubeList)
				|| dataSourceInfo == null) {
			LOGGER.info("cubeList or dataSourceInfo in param list is null: [cubeList]:["
					+ cubeList + "][dataSourceInfo]:[" + dataSourceInfo + "]");
			return result;
		}
	...
	public List<IndexMeta> getIndexMetasByDataSourceKey(String dataSourceKey) {
		...
		List<IndexMeta> metaList = super.getStoreMetaListByStoreKey(
				IndexMeta.getDataStoreName(), dataSourceKey);
	 ...
	 */
	private Map<String, List<IndexMeta>> getIndexMetasByKeyType(
			IndexMetasKeyType type, String storeKey) {
	...
	@Override
	public List<IndexShard> getIndexShardListByProductLine(
			String productLineName, String storeKey) {
		...
		if (idxMetaMap.containsKey(productLineName)) {
			...
			LOGGER.info("can not find indexMetas for productLineName:["
					+ productLineName + "]");
			return null;
		}
	...
	@Override
	public List<IndexMeta> getIndexMetasByFactTableName(String factTableName,
			String storeKey) {
		if (factTableName == null || storeKey == null
				|| factTableName.equals("") || storeKey.equals("")) {
		...
		if (idxMetaMap.containsKey(factTableName)) {
			...
			LOGGER.info("can not find indexMetas for factTable:["
					+ factTableName + "]");
			return null;
		}
	...
	public IndexMeta getIndexMetaByCubeId(String cubeId, String storeKey) {
		if (StringUtils.isEmpty(cubeId) || StringUtils.isEmpty(storeKey)) {
			LOGGER.info("can not find Cube:[CubeId:" + cubeId
					+ "] in Store:[StoreKey:" + storeKey + "]");
			return null;
		}
		Map<String, List<IndexMeta>> idxMetaMap = getIndexMetasByKeyType(
		...
		if (CollectionUtils.isNotEmpty(idxMeta.getIdxShardList())) {
			for (IndexShard idxShard : idxMeta.getIdxShardList()) {
				...
				LOGGER.info(String.format(
						...
						"[indexMeta:" + idxMeta + "][idxShard:"
								+ idxShard.getShardName() + "]", "saving node"));

			}
		}
		boolean result = super.saveOrUpdateMetaStore(idxMeta,
	...
	private class IndexMetaSimilarityScore implements Comparable {
		...
		public boolean equals(Object obj) {
			if (obj != null && (obj instanceof IndexMetaSimilarityScore)) {
				...
				if ((this.dimScore == sobj.dimScore)
						&& (this.measureScore == sobj.measureScore)
			...
			if (this.equals(o)) {
				...
				if (o != null && (o instanceof IndexMetaSimilarityScore)) {
					...
					if (this.dimScore > so.dimScore) {
						return 1;
					} else if ((this.dimScore == so.dimScore)
							&& (this.totalScore > so.totalScore)) {
						return 1;
					}
				}
			}
	...
	private Map<IndexMetaSimilarityScore, List<IndexMeta>> getMostSimilarIndexMeta(
		...
		for (IndexMeta currIdxMeta : idxMetaList) {
			IndexMetaSimilarityScore currScore = getSimilarityOfIndexMeta(
					idxMeta, currIdxMeta);
	 ...
	 */
	private IndexMetaSimilarityScore getSimilarityOfIndexMeta(
			IndexMeta idxMeta1, IndexMeta idxMeta2) {
		int dimScore = 0;
		int measureScore = 0;
		if (idxMeta1 == null
				|| idxMeta2 == null
	 ...
	 */
	private <T> Collection<T> getIntersectionOf2Collection(Collection<T> col1,
			Collection<T> col2) {
	 ...
	 */
	private List<Node> getNodeListForExistIndexShard(
			List<IndexShard> idxShardList, String clusterName) {
		List<Node> nodeList = new ArrayList<Node>();
		if (idxShardList != null && !StringUtils.isEmpty(clusterName)) {
			for (IndexShard idxShard : idxShardList) {
				List<Node> idxShardNodeList = this.isNodeService
						.getAvailableNodeListByIndexShard(idxShard, clusterName);
	...
	public IndexMeta mergeIndexMeta(IndexMeta indexMeta) {
		...
		IndexMeta idxMeta = this.getIndexMetaByIndexMetaId(
		...
		Map<String, List<IndexMeta>> factTableIdxMetaMap = this
				.getIndexMetasByKeyType(IndexMetasKeyType.FACT_TABLE,
						idxMeta.getStoreKey());
		...
		if (factTableIdxMetaMap != null
			...
			IndexMetaSimilarityScore mainScore = new IndexMetaSimilarityScore(
					idxMeta.getDimSet().size(), idxMeta.getMeasureSet().size());
			...
			if (mostSimilarIndexMetaMap != null
					&& mostSimilarIndexMetaMap.size() != 0) {
				for (IndexMetaSimilarityScore mScore : mostSimilarIndexMetaMap
						.keySet()) {
					if (mScore.compareTo(mainScore) == 0) {
						// 合并策略：0.维度、指标完全相同，直接复用
						IndexMeta currIdxMeta = mostSimilarIndexMetaMap.get(
								mScore).get(0);
						currIdxMeta.getCubeIdSet().addAll(
								idxMeta.getCubeIdSet());
						if (currIdxMeta.getIdxState().equals(
								IndexState.INDEX_AVAILABLE)) {
							currIdxMeta
									.setIdxState(IndexState.INDEX_AVAILABLE_MERGE);
						}
						...
						IndexMeta currIdxMeta = mostSimilarIndexMetaMap.get(
								mScore).get(0);
						currIdxMeta.getCubeIdMergeSet().addAll(
								idxMeta.getCubeIdSet());
						currIdxMeta.getMeasureInfoMergeSet().addAll(
								idxMeta.getMeasureSet());
						// 设置currIdxMeta的状态为需要合并
						currIdxMeta
								.setIdxState(IndexState.INDEX_AVAILABLE_NEEDMERGE);
						idxMeta = currIdxMeta;
		...
		if (idxMeta.getIdxShardList() == null
			...
			for (Node node : assignedNodeMap.keySet()) {
				...
				for (int i = 0; i < currNodeShardNum; i++) {
					IndexShard idxShard = new IndexShard(sb.toString()
							+ shardId.toString(), node);
					...
					// 只设置一次
					String idxFilePathPrefix = idxMeta
							.getIndexMetaFileDirPath();
...
//			throw new IllegalArgumentException();
//		}
//		FileUtils.write(node.getImageFilePath(),
//				SerializationUtils.serialize(node), true);
//		LOGGER.info(String.format(LogInfoConstants.INFO_PATTERN_FUNCTION_END,
//				"saveNodeImage", "[node:" + node + "]"));
//	}
	...
	public boolean saveIndexMetaLocally(IndexMeta idxMeta) throws Exception {
		...
		if (idxMeta == null) {
			LOGGER.info(String.format(
					LogInfoConstants.INFO_PATTERN_FUNCTION_ERROR,
					"saveIndexMetaLocally", "[idxMeta:" + idxMeta + "]"));
			return false;
		}
		...
		if (idxMetaFileDir.isDirectory() && idxMetaFileDir.exists()) {
			// 如果是目录并且存在
			String idxMetaImageNewFileName = idxMetaFileDir + File.separator
					+ idxMeta.getIndexMetaId()
					+ IndexFileSystemConstants.INDEX_META_IMAGE_FILE_NEW;
			String idxMetaImageBakFileName = idxMetaFileDir + File.separator
					+ idxMeta.getIndexMetaId()
					+ IndexFileSystemConstants.INDEX_META_IMAGE_FILE_BAK;
			String idxMetaImageFileName = idxMetaFileDir + File.separator
					+ idxMeta.getIndexMetaId()
					+ IndexFileSystemConstants.INDEX_META_IMAGE_FILE_SAVED;
			String idxMetaStr = AnswerCoreConstant.GSON.toJson(idxMeta,
			...
			Boolean wresult = FileUtils.write(idxMetaImageNewFileName,
					idxMetaStr.getBytes(), Boolean.TRUE);
			if (wresult) {
				...
				LOGGER.info(String
						.format(LogInfoConstants.INFO_PATTERN_FUNCTION_PROCESS_NO_PARAM,
								"saveIndexMetaLocally",
								"copyFile("
										+ idxMetaImageNewFile.getAbsolutePath()
										+ " --> "
			...
			LOGGER.info(String.format(
					LogInfoConstants.INFO_PATTERN_FUNCTION_ERROR,
					"saveIndexMetaLocally",
					"[idxMeta:" + idxMeta + "][idxMetaFileDir:"
							+ idxMetaFileDir.getAbsolutePath()
							+ " does not exist]"));
		}
		LOGGER.info(String.format(LogInfoConstants.INFO_PATTERN_FUNCTION_END,
				"saveIndexMetaLocally", "[idxMeta:" + idxMeta + "]"));
	 ...
	 */
	@Override
	public List<IndexMeta> loadIndexMetasLocalImage(String idxBaseDir,
			String currNodeKey, String clusterName) {
		LOGGER.info(String.format(LogInfoConstants.INFO_PATTERN_FUNCTION_BEGIN,
				"loadIndexMetasLocalImage", "[idxBaseDir:" + idxBaseDir
						+ "][currNodeKey:" + currNodeKey + "][clusterName:"
						+ clusterName + "]"));
		List<IndexMeta> result = new ArrayList<IndexMeta>();
		if (!StringUtils.isEmpty(idxBaseDir)
				&& !StringUtils.isEmpty(currNodeKey)) {
			...
			if (FileUtils.isEmptyDir(idxBaseDirFile)) {
				...
				String[] fileNames = idxBaseDirFile
						.list(new FileUtils.LocalImageFilenameFilter(
								IndexFileSystemConstants.INDEX_META_IMAGE_FILE_SAVED));
				for (String filePath : fileNames) {
					byte[] fileByte = FileUtils.readFile(idxBaseDir
				...
				for (String currDsDir : idxBaseDirFile.list()) {
					// 遍历每个数据源，查看索引数据
					List<IndexMeta> tmpResult = loadIndexMetasLocalImage(
							idxBaseDir + File.separator + currDsDir,
		...
		LOGGER.info(String.format(LogInfoConstants.INFO_PATTERN_FUNCTION_END,
				"loadIndexMetasLocalImage", "[idxBaseDir:" + idxBaseDir
						+ "][currNodeKey:" + currNodeKey + "][clusterName:"
						+ clusterName + "][result.size:" + result.size() + "]"));
		return result;
	}

	@Override
	public void recoverLocalIndexMetaWithCluster(List<IndexMeta> idxMetaList,
			String clusterName) {
		LOGGER.info(String.format(LogInfoConstants.INFO_PATTERN_FUNCTION_BEGIN,
				"recoverLocalIndexMetaWithCluster", "[idxMetaList.size:"
						+ idxMetaList + "][clusterName:" + clusterName + "]"));
		if (!CollectionUtils.isEmpty(idxMetaList)
				&& !StringUtils.isEmpty(clusterName)) {
			for (IndexMeta idxMeta : idxMetaList) {

				IndexMeta remoteMeta = this.getIndexMetaByCubeId(idxMeta
						.getCubeIdSet().toArray(new String[0])[0], idxMeta
						.getStoreKey());
				if (remoteMeta == null) {
					// 1. 集群中不存在，则直接恢复
					this.saveOrUpdateIndexMeta(idxMeta);
					LOGGER.info(String
							.format(LogInfoConstants.INFO_PATTERN_FUNCTION_PROCESS_NO_PARAM,
									...
									"can not find idxMetaId:"
											+ idxMeta.getIndexMetaId()
											+ " from cluster,save it directly"));
				} else {
					// 2. 集群中存在，合并索引分片
					for (IndexShard localIndexShard : idxMeta.getIdxShardList()) {
						if (remoteMeta.getIdxShardList().contains(
								localIndexShard)) {
							// 2.1
							// remoteMeta中存在该分片，则将该分片的nodeKey加到remoteMeta对应分片的机器列表中
							int idx = remoteMeta.getIdxShardList().indexOf(
									localIndexShard);
							IndexShard remoteShard = remoteMeta
									.getIdxShardList().get(idx);
							if(!remoteShard.getNodeKey().equals(localIndexShard.getNodeKey()) && !remoteShard.getReplicaNodeKeyList().contains(localIndexShard.getNodeKey())){
	 ...
	 * com.baidu.rigel.biplatform.tesseract.isservice.index.service.IndexMetaService
	 * #
	 * getSelectList(com.baidu.rigel.biplatform.tesseract.isservice.meta.IndexMeta
	 * , boolean)
	 */
	@Override
	public Set<String> getSelectList(IndexMeta indexMeta, boolean needMerge) {
		IndexMeta idxMeta = indexMeta;
		Set<String> selectList = new HashSet<String>();
		if (!CollectionUtils.isEmpty(idxMeta.getDimSet())) {
			for (String dimKey : idxMeta.getDimSet()) {
				selectList.add(dimKey);
			}
		}

		if (!CollectionUtils.isEmpty(idxMeta.getMeasureSet())) {
			for (String measureKey : idxMeta.getMeasureSet()) {
				selectList.add(measureKey);
			}
		}

		if (needMerge) {
			if (!CollectionUtils.isEmpty(idxMeta.getDimInfoMergeSet())) {
				for (String dimKey : idxMeta.getDimInfoMergeSet()) {
					selectList.add(dimKey);
				}
			}

			if (!CollectionUtils.isEmpty(idxMeta.getMeasureInfoMergeSet())) {
				for (String measureKey : idxMeta.getMeasureInfoMergeSet()) {
					selectList.add(measureKey);
				}
			}
		}
	...
	/* (non-Javadoc)
	 * @see com.baidu.rigel.biplatform.tesseract.isservice.index.service.IndexMetaService#unLockIndexMeta(com.baidu.rigel.biplatform.tesseract.isservice.meta.IndexMeta)
	 */
	@Override
